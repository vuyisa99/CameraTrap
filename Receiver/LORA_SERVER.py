...
class mylora(LoRa):
    ...
    def on_rx_done(self):
        ...
        print ("Receive: ")
        reply = bytes(payload).decode("utf-8",'ignore') # Receive DATA
        reply = reply[2:-1]
...
#         self.set_mode(MODE.TX)
    ...
    def start(self):          
        while True:
            while (self.var==0):
                #print ("Send: INF")
                action = input("Choose BITE or SQL: ")
                #print('('+action+')')
                time.sleep(1)
                if action=='SQL':
                    user_query = input("Enter your SQL query(use correct case): ")
                    user_query= user_query.encode("ascii")
                    pay_load_list = [255, 255, 0, 0] + list(user_query) + [0]
                    self.write_payload(pay_load_list) #Send query/ 
                    ...
                    ser_query = action
                    ser_query= ser_query.encode("ascii")
                    pay_load_list = [255, 255, 0, 0] + list(ser_query) + [0]
                    self.write_payload(pay_load_list) #Send query/ 
                    #self.write_payload([255, 255, 0, 0, 73, 78, 70, 0]) # Send INF
                    self.set_mode(MODE.TX)
